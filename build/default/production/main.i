...
# 43 "/opt/microchip/mplabx/v5.50/packs/Microchip/dsPIC33CK-MC_DFP/1.3.56/xc16/bin/../support/generic/h/xc.h" 3 4
# 1 "/opt/microchip/xc16/v2.00/bin/bin/../../support/generic/h/builtins.h" 1 3 4
# 44 "/opt/microchip/xc16/v2.00/bin/bin/../../support/generic/h/builtins.h" 3 4
void __builtin_write_OSCCONL(
        uint16_t);
void __builtin_write_OSCCONH(
        uint16_t);
...
# 11 "AX5043_NBM/inc/radio.h" 2
# 1 "AX5043_NBM/inc/AX5043/ax5043_interface.h" 1
# 12 "AX5043_NBM/inc/radio.h" 2
...
# 24 "AX5043_NBM/inc/radioCommands.h"
...
uint8_t radio_reset()
{
 uint32_t i;
    ...
    uint8_t data[1] = { 0x80 };
 AX5043WriteLongAddress(0, 0x002, data, 1);
    data[0] = 0x00;
 AX5043WriteLongAddress(0, 0x002, data, 1);
...
int main(void) {
    ...
    RadioInitBaseConfiguration(0);

    radioBBaseConfigs.radio = 0;
 radioBBaseConfigs.radioXTAL = 16000000;
...
uint8_t RadioWritePacket(uint8_t radio, uint8_t data[], uint8_t dataLength) {
 ...
 AX5043FIFOSetFIFOStatCommand(radio, FIFOStat_ClearFIFO);
 ...
 AX5043FIFOSetFIFOStatCommand(0, FIFOStat_ClearFIFO);
...
uint8_t make_packet(uint8_t * data, uint8_t size) {
    ...
    while(data_cnt < size) {
        ...
        pFrame[0] = (START_SYNC >> 4) & 0xFF;
        pFrame[1] = (START_SYNC) & 0xFF;
        for(uint8_t i = 0; i < 15; i++) {
            pFrame[2 + i] = (tmp_data[i*2] & 0x0F) | ((tmp_data[i*2+1] & 0x0F) << 4);
        }
        if(data_cnt != size){
            pFrame[17] = (START_SYNC >> 4) & 0xFF;
            pFrame[18] = (START_SYNC) & 0xFF;
        } else {
            pFrame[17] = (STOP_SYNC >> 4) & 0xFF;
