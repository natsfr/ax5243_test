...
uint32_t AX5043RXTrackingDatarate(uint8_t interfaceID) {
	...
	datarate += ((uint32_t)config & TRKDATARATE1_MASK) << 8;
	AX5043ReadShortAddress(interfaceID, TRKDATARATE2, &config, 1);
	datarate += ((uint32_t)config & TRKDATARATE2_MASK) << 16;
	return datarate;
}
...
int32_t AX5043RXTrackingGetRFFrequency(uint8_t interfaceID) {
	...
	frequency = ((uint32_t)config & TRKRFFREQ0_MASK) << 12;
	AX5043ReadShortAddress(interfaceID, TRKRFFREQ1, &config, 1);
	frequency |= ((uint32_t)config & TRKRFFREQ1_MASK) << 20;
	AX5043ReadShortAddress(interfaceID, TRKRFFREQ2, &config, 1);
	frequency |= ((uint32_t)config & TRKRFFREQ2_MASK) << 28;
	return (frequency >> 12);
}
