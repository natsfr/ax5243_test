...
uint8_t SPI1ReadWrite(uint8_t txByte) {
    ...
    /*SPI2BUFL = 0x0000;
    SPI2BUFH = ((uint16_t)txByte << 8) & 0xFF00;*/
    SPI2BUFL = txByte;
...
//		AX5043SynthSetPLLChargePumpCurrent(RADIO_B, 0x10);
    AX5043SynthSetPLLVCOSelection(0, 1);			//Use VCO 2 with external inductor
    AX5043SynthSetPLLVCO2Internal(0, 1);			//Use VCO 2 with external inductor
...
uint8_t RadioSetAFSKSpaceFreq(uint8_t radio, uint16_t spaceFreq) {
	...
	if(pwrMode == PwrMode_RXEN) {
		...
		uint8_t decimation = AX5043RXParamGetDecimation(radio);
		afskSpace = (uint16_t)((((uint32_t)spaceFreq * (uint32_t)decimation * (uint32_t)xtaldiv) << 16) / (float)FXTAL + 0.5f);
	}
	else {
...
uint8_t RadioSetAFSKMarkFreq(uint8_t radio, uint16_t markFreq) {
	...
	if(pwrMode == PwrMode_RXEN) {
		...
		uint8_t decimation = AX5043RXParamGetDecimation(radio);
		afskMark = (uint16_t)((((uint32_t)markFreq * (uint32_t)decimation * (uint32_t)xtaldiv) << 16) / (float)FXTAL + 0.5f);
	}
	else {
		//Configuration for AFSK TX
